...
\begin{itemize}
  \item \textbf{Customer.java} — Manages customer data and an instance of validation in the email entries.
  \item \textbf{Game.java} — Represents the game properties and their attributes.
  \item \textbf{Store.java} — Manages lists of customers and games.
  \item \textbf{Main.java} — Entry point that handles user interaction.
\end{itemize}
...
\midrule
Blevis Allushi & Co-author and developer. Designed the overall class architecture and implemented (\texttt{Customer}, \texttt{Game}) classes, implemented key methods and restrictions via enumeration, finalized (\texttt{Main}) after all work was submitted. Contributed to UML modeling and documentation structure. \\
\addlinespace
Kristjan Seraj & Co-author and developer. Implemented core functionality for connecting (\texttt{Customer}, \texttt{Game}) classes through the (\texttt{Store}) class, implemented ArrayLists, developed search and display methods. Contributed equally to debugging, testing, and UML refinement. \\
\bottomrule
\end{tabular}
\end{center}
